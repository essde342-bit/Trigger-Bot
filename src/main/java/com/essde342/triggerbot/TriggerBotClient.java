...
public class TriggerBotClient implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ...
        openMenuKey = KeyBindingHelper.registerKeyBinding(new KeyBinding(
                "key.triggerbot.open_menu", InputUtil.Type.KEYSYM,
                GLFW.GLFW_KEY_RIGHT_SHIFT, "category.triggerbot"));

        ClientTickEvents.END_CLIENT_TICK.register(client -> {
            while (openMenuKey.wasPressed()) client.openScreen(new TriggerBotScreen());
            if (client.player != null && client.world != null && CONFIG.enabled) tickTriggerBot(client);
        });
    }
    ...
    private static void tickTriggerBot(MinecraftClient client) {
        ...
        if (player.isUsingItem()) {
            ...
            if (active != null && (active.getUseAction() == UseAction.EAT || active.getUseAction() == UseAction.DRINK)) return;
        }
        ...
        if (CONFIG.onlyCrits && !canCriticalHit(player)) return;
    ...
    private static boolean canCriticalHit(PlayerEntity player) {
        return !player.isOnGround() && player.fallDistance > 0.0F
                && !player.isOnLadder() && !player.isTouchingWater()
                && !player.hasVehicle() && !player.hasStatusEffect(StatusEffects.BLINDNESS)
                && !player.isSprinting() && player.getAttackCooldownProgress(0.0F) >= 0.9F;
    }
    ...
    public static void loadConfig() {
        ...
        try (FileReader reader = new FileReader(file)) {
            ...
            CONFIG.enabled = readBoolean(text, "enabled", true);
            CONFIG.onlyCrits = readBoolean(text, "onlyCrits", false);
            CONFIG.onlyWeapon = readBoolean(text, "onlyWeapon", false);
        } catch (IOException ignored) {}
    }
