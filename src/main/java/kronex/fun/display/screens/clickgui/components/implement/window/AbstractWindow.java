package kronex.fun.display.screens.clickgui.components.implement.window;
...
public abstract class AbstractWindow extends AbstractComponent {
    protected boolean dragging, draggable;
    protected int dragX, dragY;

    @Getter
    private final kronex.fun.other.utils.display.other.animation.Animation scaleAnimation =
            new DecelerateAnimation().setValue(1).setMs(200);
    ...
    @Override public AbstractWindow size(float width, float height) { this.width = width; this.height = height; return this; }
    @Override public AbstractWindow position(float x, float y) { this.x = x; this.y = y; return this; }
    ...
    public void startCloseAnimation() { scaleAnimation.setDirection(Direction.BACKWARDS); }

    public boolean isCloseAnimationFinished() { return scaleAnimation.isFinished(Direction.BACKWARDS); }
}