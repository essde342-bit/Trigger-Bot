...
public class ButtonComponent extends AbstractComponent {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        width = Fonts.getSize(12, Fonts.Type.DEFAULT).getStringWidth(text) + 13;
        height = 12;
        rectangle.render(ShapeProperties.create(matrix, x, y, width, height).round(2).color(color).build());
        Fonts.getSize(12, BOLD).drawCenteredString(matrix, text, x + (double) width / 2, y + 5, -1);
    }

    @Override
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        if (MathUtil.isHovered(mouseX, mouseY, x, y, width, height) && button == 0) runnable.run();
