...
public class WindowManager extends AbstractComponent {
    ...
    public void add(AbstractWindow window) { windows.add(window); }

    public void delete(AbstractWindow window) { window.startCloseAnimation(); }
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        windows.forEach(window -> {
            ...
            if (window.isCloseAnimationFinished()) toRemove.add(window);
        });
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        for (AbstractWindow window : windows) window.startCloseAnimation();
        return false;
    }

    @Override public boolean isHover(double mouseX, double mouseY) {
        for (AbstractWindow window : windows) if (window.isHovered(mouseX, mouseY)) return true;
    ...
    @Override public boolean mouseScrolled(double mouseX, double mouseY, double amount) {
        for (AbstractWindow window : windows) if (window.mouseScrolled(mouseX, mouseY, amount)) return true;
    ...
    @Override public boolean mouseDragged(double mouseX, double mouseY, int button, double deltaX, double deltaY) {
        for (AbstractWindow window : windows) if (window.mouseDragged(mouseX, mouseY, button, deltaX, deltaY)) return true;
