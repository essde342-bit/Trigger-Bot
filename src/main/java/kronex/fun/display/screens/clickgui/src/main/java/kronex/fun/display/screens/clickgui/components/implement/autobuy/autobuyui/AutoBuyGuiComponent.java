...
public class AutoBuyGuiComponent extends AbstractComponent {
    ...
    @Override public void render(DrawContext c,int mx,int my,float d){
        ...
        for(AutoBuyableItem item:items){
            if(shown>=7)break;
            int bg=item.isEnabled()?ColorAssist.applyOpacity(ColorAssist.getClientColor(),100):ColorAssist.getGuiRectColor(.7f);
            rectangle.render(ShapeProperties.create(c.getMatrices(),x+8,oy,width-16,22).round(4).color(bg).build());
            Fonts.getSize(11,Fonts.Type.DEFAULT).drawString(c.getMatrices(),item.getDisplayName(),x+14,oy+8,0xFFE8EAF0);
            Fonts.getSize(10,Fonts.Type.DEFAULT).drawString(c.getMatrices(),"$"+item.getSettings().getBuyBelow(),x+width-60,oy+8,0xFFBFC3D0);
            oy+=25;shown++;
        }
    ...
    @Override public boolean mouseClicked(double mx,double my,int button){
        List<AutoBuyableItem> items=ItemRegistry.getAllItems();
        int index=(int)((my-(y+30))/25);
        if(index>=0&&index<items.size()&&Calculate.isHovered(mx,my,x+8,y+30,width-16,175)){
            AutoBuyableItem item=items.get(index);
            if(button==0){manager.toggleItem(item);status.setStatus(item.isEnabled()?"Enabled":"Disabled",1200);return true;}
            if(button==1){windowManager.add(new AutoBuyItemSettingsWindow(item).position(mx+4,my+4).size(175,90).draggable(true));return true;}
        }
